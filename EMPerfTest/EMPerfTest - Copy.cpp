...
//argv[6]:-c
//argv[7]:context window size
//argv[8]:-s
//argv[9]:sim type:0,average;1,weighted;2,max
//argv[10]:-sw
//argv[11]:whether using stop words
//argv[12]:stop words file
//argv[13]:-tfidf
//argv[14]:whether using tf*idf weighting in test
//argv[15]:whether logging bad cases info
//argv[16]:bad cases number
//argv[17]:bad cases logging file
...
int isHs = 1, isStopWords = 0, isTfIdf = 0, contextWindowSize;
simType mysimType;
...
bool ParseCmdArgs(int argc, char* argv[])
{
	...
	for(int i = 6; i < argc; i += 2)
	{
		...
		if(strcmp(paraType, "c") == 0)
			contextWindowSize = atoi(argv[i+1]);

		else if(strcmp(paraType, "s") == 0)
		{
	...
	for(int i = 0; (c = sentence[i]) != '\0'; ++i)
	{
		...
		else if(c != '<')
		{
			...
			if(isInWord && (!isStopWords || swTrie.GetWordIndex(word) == -1))
				words[idx++] = trie.GetWordIndex(word);
			isInWord = false;
			widx = 0;
		}
		else
...
void GetCtxWords(const int* sentence, const int senLen, const int wordIdx, int* ctx, int& ctxSize)
{
	...
	ctxSize = 0;
	for(int a = 0; a < 2 * contextWindowSize + 1; ++a)
	{
		int p = wordPos - contextWindowSize + a;
		if(a == contextWindowSize || p < 0 || p >= senLen || sentence[p] == -1)
			continue;
		
		ctx[ctxSize++] = sentence[p];
	}
}
...
void ComputeAllSimScores()
{
	...
	for(int j = 0; j < DATA1_SIZE; ++j)
	{
		...
		GetPrototypeProbInContext(ehdata[j].idx0, ehdata[j].ctx0, ehdata[j].ctxSize0, prob0, j * 2);
		GetPrototypeProbInContext(ehdata[j].idx1, ehdata[j].ctx1, ehdata[j].ctxSize1, prob1, j * 2 + 1);
...
void AnalyzeResults()
{
	...
	loss = Util::CompareRanks(rankEH, rankGoldEH, DATA1_SIZE, badCases, badCaseNum1);
...
int main(int argc, char* argv[])
{
	...
	ele_type pcorrEM1 = Util::ComputeSRankCorr(rankGoldEH, rankEH, DATA1_SIZE);

	printf("ws353 score:%.4f eh score:%.4f bad count:%d\n\n", pcorrEM0, pcorrEM1, badCnt);
