...
ele_type best_special_score = -10;
...
void ComputeEHScores(int currWinSize)
{
	...
	for(int j = 0; j < DATA1_SIZE; ++j)
	{
		...
		GetPrototypeProbInContext(ehdata[j].idx0, ehdata[j].ctx0, std::min(ehdata[j].ctxSize0, 2 * currWinSize), prob0, ehdata[j].id * 2);
		GetPrototypeProbInContext(ehdata[j].idx1, ehdata[j].ctx1, std::min(ehdata[j].ctxSize1, 2 * currWinSize), prob1, ehdata[j].id * 2 + 1);
		...
		aver_eh_score += ehdata[j].simScoreWithCtx;
		if (ehdata[j].id == special_idx && ehdata[j].simScoreWithCtx > best_special_score)
			best_special_score = ehdata[j].simScoreWithCtx;
	}
...
int main(int argc, char* argv[])
{
	...
	int best_wins_size, best_special_rank = 1000000;
	for(int cwindow = 1; cwindow <= (mysimType == simType::Uniform? 1: MAX_WINDOW_SIZE); ++ cwindow)
	{
		...
		if (rankEH[special_idx] < best_special_rank)
			best_special_rank = rankEH[special_idx];
	...
	//printf("gold rank : %d, alg rank: %d, alg score:%.4f\n", rankGoldEH[special_idx], best_special_rank, best_special_score);
